...
\textcite{Bunker2017} betonten in ihrem kritischen Review, dass \textbf{ANNs dominieren}, wenn reine Vorhersagegenauigkeit das Ziel ist -- allerdings auf Kosten von Interpretierbarkeit und höherem Rechenaufwand. Für diese Arbeit heißt das: MLP wird als repräsentativer Deep-Learning-Ansatz evaluiert, obwohl Ensemble-Methoden (XGBoost, LightGBM) bei tabellarischen Daten oft effizienter sind \cite{Shwartz-ZivAramon2020}.
...
Dieser Befund hat für die eigene Methodik eine wichtige Konsequenz: Da EPV-Daten nicht verfügbar waren und saisonales Understat-xG \cite{Understat2024} eine breitere Basis bietet, liegt der Fokus auf Pre-Match-xG -- obwohl \textcite{ForcherEtAl2025} zeigen, dass xG seine Stärken vor allem im Post-Match-Setting entfaltet -- gestützt durch die Evidenz, dass xG-Features die Vorhersagegenauigkeit signifikant verbessern \cite{AnzerBauer2021,VanEetveldeEtAl2024}.
...
\textcite{VanEetveldeEtAl2024} demonstrierten, dass \textbf{xGA (Expected Goals Against)} die defensive Stabilität robuster misst als kassierte Tore, da es Glück/Pech in der Torhüterperformance herausrechnet. Für das eigene Modell bedeutet das: Exponentiell geglättete xG/xGA-Durchschnitte (EWA) erfassen die aktuelle Teamform präziser als reine Torstatistiken.
...
Deshalb setzt diese Arbeit auf eine \textbf{kombinierte Evaluation} von fünf Modellen (Logistische Regression, Random Forest, XGBoost, LightGBM, MLP) mit \textbf{xG-basierten Features} auf Bundesliga-Daten, ergänzt um eine systematische Analyse von \textbf{Dimensionsreduktionseffekten} (PCA vs.\ Feature Selection).

\textcite{GrollLeyEffenberger2019} betonten, dass die \textbf{Kombination aus statistischer Rigorosität und sportlichem Kontextwissen} den Unterschied zwischen akademischer Übung und praxisrelevanter Forschung ausmacht. Diesem Anspruch folgt auch diese Arbeit: Die Methodik (Kapitel~\ref{chap:methodik}) baut auf den hier reviewten Erkenntnissen auf und adressiert die identifizierten Forschungslücken durch eine umfassende Evaluation von xG-basierten Features im Bundesliga-Kontext.
